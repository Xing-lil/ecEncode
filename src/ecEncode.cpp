#include "ecEncode.hpp"

ecEncode::ecEncode(int n_, int k_, int maxSize_) : n(n_), k(k_), maxSize(maxSize_), m(n_ + k_)
{
    // 各种中间矩阵的空间分配
...
bool ecEncode::startEC(vvc_u8 &in, vvc_u8 &out, size_t size)
{
    size_t index = 0;
    u8 *frag_ptrs[m];
    int len = maxSize;
...
bool ecEncode::recoverEC(vvc_u8 &matrix, int err_num, u8 *err_list, size_t size)
{
