...
int main()
{
    int n = 6, k = 2, maxSize = INT_MAX; // n：数据条带数量 k：校验条带数量
    size_t len = 10000;                  // len：条带长度 2147483647 357913941
    size_t size = n * len;
    ...
    ecEncode ec(n, k, maxSize);
    // EC校验的计算
    ec.startEC(in, out, size);
    // cout << "------------------------ EC校验块 ------------------------" << endl;
    // for (int i = 0; i < k; i++)
    // {
    //     for (int j = 0; j < len; j++)
    ...
    ec.recoverEC(matrix, err_num, err_list, size);
    // cout << "---------------------- 恢复样例(恢复) ----------------------" << endl;
    // for (int i = 0; i < n + k; i++)
    // {
    //     for (int j = 0; j < len; j++)
